...
import api from '../api/axios'; // Assicurati che il percorso sia corretto in base alle tue cartelle!

const Register = () => {
  const navigate = useNavigate(); // Inizializziamo l'hook per la navigazione
  ...
  const handleSubmit = async (e) => {
    ...
    // Validazioni Frontend (ottime per evitare chiamate inutili al server)
    ...
    setIsLoading(true); // Disabilitiamo il bottone durante il caricamento
    ...
    try {
      ...
      // Se il backend risponde con successo (status 201):
      // 1. Salviamo il token nel localStorage
      localStorage.setItem('token', response.data.token);
      
      // 2. Reindirizziamo l'utente alla home (o alla dashboard)
      navigate('/'); 

    } catch (err) {
      // Catturiamo l'errore che arriva dal backend (es. "Username già in uso")
      ...
      setIsLoading(false); // Riabilitiamo il bottone
    }
  };
      ...
      <div className="bg-zinc-900 p-6 sm:p-8 rounded-2xl shadow-[0_0_20px_rgba(168,85,247,0.3)] border border-purple-500/50 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm font-medium">
            l'unica piattaforma per mostrare il tuo black-humor
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-3 p-3 bg-zinc-950 border border-purple-500/20 rounded-lg shadow-inner">
              <p className="text-xs text-gray-400 leading-relaxed">
                <span className="text-purple-400 font-bold">Requisiti di sicurezza:</span> Minimo 8 caratteri, deve includere almeno una lettera maiuscola, un numero e un carattere speciale.
              </p>
            </div>
          </div>
        ...
        <p className="text-center text-sm text-gray-400 mt-8">
          ...
          <Link to="/login" className="text-purple-400 font-bold hover:text-purple-300 hover:drop-shadow-[0_0_5px_rgba(168,85,247,0.8)] transition-all">
