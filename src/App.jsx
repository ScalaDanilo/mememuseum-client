...
import { createBrowserRouter, RouterProvider, Outlet, Link, useNavigate, useLocation } from 'react-router-dom';
...
const MainLayout = () => {
  ...
  const pillButtonStyle = "px-5 py-2 rounded-full font-bold border border-purple-500 text-purple-400 bg-transparent shadow-[0_0_10px_rgba(168,85,247,0.3)] hover:shadow-[0_0_20px_rgba(168,85,247,0.7)] hover:bg-purple-900/20 hover:text-purple-300 transition-all text-sm sm:text-base whitespace-nowrap";
              ...
              <div className="relative">
                ...
                {isFilterOpen && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-4 w-56 bg-zinc-900 border border-purple-500/50 rounded-xl shadow-[0_10px_30px_rgba(168,85,247,0.3)] overflow-hidden z-50 flex flex-col">
                    {!showTagInput ? (
                      <>
                        <button onClick={() => applyFilter('sort', 'date_desc')} className="px-4 py-3 text-left text-sm hover:bg-purple-900/30 hover:text-purple-300 border-b border-purple-500/20 transition-colors">Più Recente</button>
                        <button onClick={() => applyFilter('sort', 'date_asc')} className="px-4 py-3 text-left text-sm hover:bg-purple-900/30 hover:text-purple-300 border-b border-purple-500/20 transition-colors">Meno Recente</button>
                        <button onClick={() => applyFilter('sort', 'most_upvoted')} className="px-4 py-3 text-left text-sm hover:bg-purple-900/30 hover:text-purple-300 border-b border-purple-500/20 transition-colors">Più UpVote</button>
                        <button onClick={() => applyFilter('sort', 'most_downvoted')} className="px-4 py-3 text-left text-sm hover:bg-purple-900/30 hover:text-purple-300 border-b border-purple-500/20 transition-colors">Meno UpVote</button>
                        <button onClick={() => setShowTagInput(true)} className="px-4 py-3 text-left text-sm hover:bg-purple-900/30 hover:text-purple-300 font-bold text-purple-400 transition-colors">Cerca per Tag...</button>
                      </>
                    ) : (
                      <div className="p-3 flex items-center gap-2">
                        <input
                          type="text" autoFocus placeholder="Es. Gatto..." value={tagSearch} onChange={(e) => setTagSearch(e.target.value)}
                        ...
                        />
                        <button onClick={() => tagSearch.trim() && applyFilter('tag', tagSearch.trim())} className="bg-purple-600 p-2 rounded-lg hover:bg-purple-500 transition-colors">
                          <Search size={16} />
                        </button>
                      </div>
                    )}
            ...
            <div className="order-2 lg:order-3 shrink-0">
              {isAuthenticated ? (
                <Link to="/profile" className="flex items-center justify-center w-10 h-10 bg-zinc-900 rounded-full border border-purple-500 hover:bg-purple-900/20 transition-all shadow-[0_0_10px_rgba(168,85,247,0.3)] hover:shadow-[0_0_20px_rgba(168,85,247,0.6)] overflow-hidden">
                  {userAvatar ? (
                    <img src={`${BACKEND_URL}${userAvatar}`} alt="Avatar" className="w-full h-full object-cover" />
                  ) : (
                    <User className="text-purple-400" size={20} />
                  )}
                </Link>
              ) : (
                <Link to="/login" className="px-4 py-2 sm:px-5 sm:py-2 rounded-full font-bold border border-purple-500 text-purple-400 bg-transparent shadow-[0_0_10px_rgba(168,85,247,0.3)] hover:shadow-[0_0_20px_rgba(168,85,247,0.7)] hover:bg-purple-900/20 hover:text-purple-300 transition-all text-sm whitespace-nowrap">
                  Accedi
                </Link>
              )}
            </div>
